...
class LWEGScharrModule(nn.Module):
    ...
    def __init__(self, channel, norm_layer="BN", act_layer=nn.ReLU):
        super().__init__()
        # Initialize with Scharr filters but make them learnable
        ...
        # Initialize with Scharr filters but keep them learnable
        with torch.no_grad():
...
class LWEGGaussianModule(nn.Module):
    ...
    def __init__(self, dim, size=5, sigma=1.0, norm_layer="BN", act_layer=nn.ReLU):
        super().__init__()
        # Create learnable Gaussian convolution
        self.gaussian = nn.Conv2d(dim, dim, kernel_size=size, stride=1, padding=size//2, groups=dim, bias=False)
        
        # Initialize with Gaussian kernel but keep it learnable
        gaussian_kernel = self.gaussian_kernel(size, sigma)
        with torch.no_grad():
            self.gaussian.weight.data = gaussian_kernel.repeat(dim, 1, 1, 1)
...
class BlurAwareModule(nn.Module):
    ...
    def __init__(self, channels: int, use_lweg: bool = False):
        ...
        if use_lweg:
            ...
            # Create a dummy module to maintain parameter consistency
            self.gaussian_detector = nn.Identity()
        ...
        # Always use adaptive threshold to ensure parameter usage
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Always use both paths to ensure gradient flow
        if self.use_lweg:
            ...
            # Ensure gaussian detector is used (with zero contribution)
            dummy_gaussian = self.gaussian_detector(x)
            blur_response = blur_response + dummy_gaussian * 0.0
        
        # Always use threshold - use sigmoid for smooth gradient flow
        threshold = self.threshold(x)
        # Use sigmoid instead of hard threshold for better gradient flow
        blur_mask = torch.sigmoid((threshold - blur_response) * 10.0)  # Scale factor for sharpness
...
class HybridDistortionAwareNeck(nn.Module):
    ...
    def forward(self, feats: List[torch.Tensor]) -> List[torch.Tensor]:
        ...
        # Optional enhancements with safety mechanism for DDP
        if hasattr(self, 'edge_enhancers') or hasattr(self, 'blur_enhancers'):
            enhanced_feats = []
            for i, feat in enumerate(proj_feats):
                enhanced = feat
                
                # Edge enhancement with safety
                if hasattr(self, 'edge_enhancers'):
                    ...
                    # Ensure adaptive_fusion is used even when edge enhancement is disabled
                    if hasattr(self, 'adaptive_fusion'):
                        if isinstance(self.adaptive_fusion[i], LWEGLFEAModule):
                            # Use identity for edge_enhanced when no edge enhancement
                            enhanced = self.adaptive_fusion[i](enhanced, enhanced)
                        else:
                            weight = self.adaptive_fusion[i](enhanced)
                            # Apply weight but with same input (no change but gradient flow)
                            enhanced = enhanced * (1 - weight) + enhanced * weight
                
                # Blur enhancement with safety
            ...
            # Safety: Ensure unused enhancement modules still get gradient flow
            # This prevents DDP issues when enhancement modules exist but are not used
            if hasattr(self, 'edge_enhancers'):
                for i, enhancer in enumerate(self.edge_enhancers):
                    dummy_output = enhancer(proj_feats[i])
                    proj_feats[i] = proj_feats[i] + dummy_output * 0.0  # Zero contribution but gradient flow
            
            if hasattr(self, 'blur_enhancers'):
                for i, enhancer in enumerate(self.blur_enhancers):
                    dummy_output = enhancer(proj_feats[i])
                    proj_feats[i] = proj_feats[i] + dummy_output * 0.0  # Zero contribution but gradient flow
            
            # Ensure adaptive_fusion gets gradient flow even when not actively used
            if hasattr(self, 'adaptive_fusion'):
                for i, fusion in enumerate(self.adaptive_fusion):
                    if isinstance(fusion, LWEGLFEAModule):
                        dummy_output = fusion(proj_feats[i], proj_feats[i])
                        proj_feats[i] = proj_feats[i] + dummy_output * 0.0
                    else:
                        dummy_weight = fusion(proj_feats[i])
                        proj_feats[i] = proj_feats[i] + dummy_weight.sum() * 0.0
